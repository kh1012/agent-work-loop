...
시간 내 우편함으로 안 돌아오는 지연이 실전에서 반복 관측됐다(원인 미확정). 무한정 기다리지 않는다
— 서브에이전트에게 넘겼던 `exec/<name>.md`·`plan/<name>.taken.md`를 메인이 직접 열어 커밋·완료조건을
대조하고, 판정을 메인이 직접 내려 위 3(판정)을 진행한다(pipeline-spawned-subagent-lifecycle).
