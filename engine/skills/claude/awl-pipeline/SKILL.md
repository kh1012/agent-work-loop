...
- **필수 부트스트랩 절차 — 선제적 상태체크를 즉시 예약한다.** exec·review를 스폰한 **직후**, 오케스트레이터는 `ScheduleWakeup`으로 20~30분의 고정 주기 상태체크를 즉시 예약한다. 이 예약은 idle 알림을 기다려 시작하는 사후 대응이 아니다. 알림이 활발히 오거나 `SendMessage`로 재개를 시킨 동안에도 다음 상태체크를 계속 재예약한다. 각 틱에서 exec·review의 우편함·워처·진행 상태를 확인하고 필요하면 재개시킨다. 이 주기 예약은 레인 전체 파이프라인이 완료된 경우에만 멈춘다 — 알림이 끊긴 순간 exec·review가 함께 유휴에 빠져도 다음 틱이 반드시 둘을 깨운다.
  - **재예약 기준은 마지막 예약 시각이 아니라 마지막 실제 활동 시각이다.** 상태체크 예약 사이에 `SendMessage` 왕복이 활발히 있었다면, 다음 상태체크는 "직전 예약 + N분"이 아니라 "그 마지막 왕복 시각 + N분"으로 다시 계산한다. 그러지 않으면 예약된 프롬프트가 발화 시점엔 이미 여러 사이클만큼 낡은 지시를 담고 있어(구식 인자), 오케스트레이터가 그때마다 실제 상태를 다시 확인하는 불필요한 왕복이 붙는다(pipeline-orchestrator-overhead).
  - **주기 상태체크의 `ScheduleWakeup` 프롬프트에 `/awl-pipeline` 트리거 문구를 다시 넣지 않는다.** 스킬은 이미 이 세션 컨텍스트에 있다 — 트리거 문구를 다시 쓰면 스킬 전체 본문이 매번 재주입된다. 상태체크 틱의 프롬프트는 "exec/review 우편함·워처 확인, 상태 변화 없으면 결과만 다음 실질 이벤트에 합쳐 보고" 같은 평서문 지시로 충분하다. 그 틱에서 실제로 판단·개입이 필요해지면(게이트, 막힘 등) 그때 스킬 본문을 참조하면 된다 — 매 틱 선제 재주입이 아니라 필요할 때만 재주입한다.
...
- **세션 주기 교체(pipeline-worker-rotation) — 컨텍스트 무한 누적 방지.** 매 턴 전체 대화 이력을
  다시 읽는 특성상, 같은 exec·review 세션을 워크아이템 수십 개에 걸쳐 계속 재사용하면 턴 비용이
  단조 증가한다. role별로 **완료한 워크아이템이 8개 누적될 때마다** 그 세션을 `TaskStop`으로
  종료하고, 같은 스킬 트리거(`Skill(awl-pipeline-exec)`/`Skill(awl-pipeline-review)`)로 새 세션을
  스폰해 카운터를 0으로 되돌린다. 이 카운터는 "사이클 완료 요약"의 `cycleAgentCount`(오케스트레이터의
  스폰 횟수, 사이클마다 리셋)와 별개다 — 워커 하나가 여러 사이클에 걸쳐 재사용될 수 있으므로, role별
  누적 완료 워크아이템 수를 독립적으로 센다. **교체는 반드시 워크아이템 경계에서만** 한다 — 커밋도
  핸드오프도 안 된 WIP를 든 채로는 교체하지 않는다(다음 워크아이템으로 넘어가기 직전까지 미룬다).
  교체 직후 스폰 프롬프트는 기존 "좁은 범위·절대경로" 원칙 그대로, "방금 세션이 교체됐다"는 사실과
  현재 처리할 워크아이템, 레인 절대경로만 담는다 — 이전 세션의 이력을 요약해 넘기지 않는다(모든 실질
  상태는 파일 기반이라 새 세션이 부트스트랩만으로 이어받는다, 위 "세션 완전소실 복구절차" 참고).
- **dispatch envelope 발급과 워커 완료 보고의 시간 교차는 정상이다.** 오케스트레이터의 응답 지연과
  워커의 빠른 완료가 겹치면 "이미 보냈다/교차했다"류 해명이 반복될 수 있다 — 이건 스폰 계약 위반이
  아니라 비동기 타이밍의 자연스러운 결과다. 구조적으로 줄일 방법은 없다(워커가 완료 직전 신호를
  미리 보내게 하는 프로토콜은 이미 표준인 "완료 즉시 다음 envelope 발급"보다 나을 게 없다) — 교차가
  나면 그냥 최신 상태를 다시 확인하고 넘어간다.
...
- **review envelope는 exec의 완료 보고를 받은 뒤에만 발급한다(doc-only-round-and-foreign-listener-provenance
  AC-07).** exec가 핸드오프 파일을 계속 고치는 도중에(예: 마지막 저장 직후) 코디네이터가 파일
  내용을 미리 해시해 envelope를 발급하면, 그 직후 exec가 한 번 더 저장하는 순간 해시가 어긋나
  claim이 `DISPATCH_INPUT_HASH_MISMATCH`로 거부된다 — tamper가 아니라 정상 흐름에서 반복될 수
  있는 경합이다. 이 경합의 안전한 해소는 **발급 시점을 늦추는 것**이지 해시 비교를 느슨하게(예:
  mtime+size 병기, 불일치 시 자동 재발급) 만드는 것이 아니다 — 후자는 실제 tamper 시도도 같은
  경로로 통과시켜 이 메커니즘의 존재 이유(무결성 보장)를 무너뜨린다. 그래서: 위 "완료 시 전체
  결과를 본문에 담아 보내라"는 완료 보고를 **실제로 받은 뒤에만** review envelope의 해시를 뜬다 —
  파일 존재 여부 폴링이나 워처 신호만으로 앞서 발급하지 않는다. 그래도 경합이 나면(해시 불일치)
  코디네이터가 최신 내용으로 새 envelope를 재발급한다 — 이건 이미 정상 대응 경로다.
...
- **idle-only 이벤트는 사람에게 즉시 보고하지 않는다.** 상태체크 틱이나 워커 idle 알림이 "작업
  없음/변화 없음" 확인일 뿐이면 그 자체로 사람에게 보고하지 않고 다음 실질 이벤트(워크아이템 완료,
  게이트, 막힘, 사이클 종료)가 생겼을 때 그동안의 idle 구간을 한 줄로 합쳐 보고한다("N분간 유휴,
  변화 없음" 정도). 워크아이템 완료·게이트·에스컬레이션처럼 상태가 실제로 바뀐 이벤트는 이 절 앞부분의
  표 우선 원칙대로 즉시 보고한다.
- **워커(팀메이트) 대면 메시지도 짧게 쓴다.** exec·review에게 보내는 재개·지시 메시지는 지시와
  필요한 데이터(경로·id)를 먼저 쓰고, 배경 재서술은 이전 지시와 달라진 부분만 남긴다. 칭찬·격려·
  장문의 근거 나열은 사람 대면 보고에만 쓰고 워커 대면 메시지에는 넣지 않는다 — 워커는 이미 자기
  작업 맥락을 파일 기반 상태로 갖고 있어 지시·데이터만으로 충분한 경우가 많다.
