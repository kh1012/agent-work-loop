...
**피드백 플러시는 조건부 여지가 아니라 필수 게이트다.** 한 일감(1 또는 2)을 끝낸 시점에 더
처리할 대상이 없다면 — "일감을 끝냈으니 여기서 마친다"고 턴을 바로 끝내지 않는다. 반드시 이
4번까지 도달해 순서대로 밟는다: 피드백 모드가 켜져 있고 이번 사이클에서 누적한 관찰이 있으면
**그 즉시**(다음 턴으로 미루지 않는다 — 다음 턴이 언제 올지 보장 없다) awl-pipeline "피드백
모드" 절의 경로·형식대로 한 번에 정리해 기록한다(관찰이 없으면 아무것도 안 쓴다 — 매 유휴마다
빈 파일을 만들지 않는다). 그다음 워처를 1회 체크하고, 없으면 다음 확인을 예약한 뒤 턴을 끝낸다
(아래 "self-pace").
...
구현이나 검증이 TCP 포트를 여는 서비스를 시작할 때:
