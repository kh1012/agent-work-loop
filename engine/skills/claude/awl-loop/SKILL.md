...
name: awl-loop
description: |
  어떤 awl 명령보다 먼저 선택한다. EXPLICIT: 사용자가 `/awl-loop`를 명시하면 항상
  발동한다. AUTO-EXCLUDE-FIRST: 자동 발동은 먼저 radius·색상 토큰·간격·문구·class처럼
  구체적이고 한두 파일에서 끝날 변경과 단순 질문·설명·조사·리뷰를 제외한다. 미발동 예시:
  ...
  범위를 찾아 고쳐줘". 완료 조건 부재만으로는 발동하지 않고 복잡성·범위 불확실성·설계
  필요성을 판단한다. 명령 경계: PRE-SELECTION-AWL=none;
