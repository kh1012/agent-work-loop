...
auto        semi-auto 와 같다(이 스킬은 아직 게이트 3·4 를 안 쓰는 레거시 2게이트 흐름이라,
            "게이트 4 에서 펼친 요약" 같은 auto 전용 동작은 4게이트 티켓 모델로 이관되기 전까지
            의미가 없다 — 그때까지 semi-auto 와 동일하게 취급한다)
```

**이 레거시 2게이트 흐름에서는 게이트 1(계획 승인)을 어느 모드에서도 사람에게 묻는다** —
"요청당 1회"라 부담이 적고, 구현을 시작하기 전 마지막 확인이라서다. 아래 "게이트 2" 절의
조건분기가 실제 자동승인 지점이다.

> **4게이트 티켓 모델(`/awl`)에서는 다르다.** 설계(adk-prototype.md:365)의 `auto` 는
> "전부 자동"이라 게이트 1 도 자동 승인된다. 위 예외는 **이 스킬의 레거시 흐름 한정**이고,
> `/awl` 쪽 규칙은 `engine/skills/claude/awl/SKILL.md` 가 정본이다. 두 스킬이 같은 모드
> 이름을 쓰지만 게이트 1 동작이 다르다는 점을 기억할 것.
