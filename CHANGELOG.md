...
- 티켓이 없어도 `awl next` 가 지침을 낸다. 캐묻기 강도와 spec·clarification 자리를
  이때 알려준다 — 캐물어야 할 바로 그 순간에 출력이 없던 문제
- `awl record` 가 스펙을 소유자로 받는다. 요청 레이어인 게이트 1·4 는 티켓이 없어서
  여태 기록할 수 없었다
- 진행할 티켓이 없는 이유를 구분해서 말한다. 전부 done 이면 게이트 4 로, 막혀 있으면
  무엇을 기다리는지, 티켓이 아예 없으면 스펙부터. 셋을 뭉뚱그려 "티켓 없음"이라
  하던 탓에 끝난 요청을 닫지 않고 새 요청을 파는 일이 있었다
- 기록이 가리키는 스펙 이름을 `spec` 으로 통일했다. 쓰는 쪽과 읽는 쪽 이름이 달라
  `awl next` 의 "이미 아는 것"이 늘 비어 있었다
- 스펙 오타를 티켓과 함께 줘도 잡는다
- 프로파일을 못 읽으면 스킬 자리가 조용히 사라지지 않고 이유를 남긴다

## [0.9.8] - 2026-07-29

### 변경

- 자리별 스킬·모드별 강도·링크 점검 반영

## [0.9.7] - 2026-07-29

### 변경

- 단계별 기본 스킬과 awl doctor --links 문서화

## [0.9.6] - 2026-07-29

### 추가

- 스킬 링크 점검 — 자리가 가리키는 게 실제로 있는지 본다

## [0.9.5] - 2026-07-29

### 추가

- 자리마다 기본 스킬 + 모드가 캐묻기·마감 강도를 정한다

## [0.9.4] - 2026-07-29

### 변경

- 0.6.x~0.9.x 구간을 채우고 대체된 결정을 표시

## [0.9.3] - 2026-07-29

### 변경

- commands.md·storyline.md 를 0.9.x 기준으로 다시 씀

## [0.9.2] - 2026-07-29

### 변경

- 제품 문서 형태로 재편, 상대경로 링크 제거

## [0.9.1] - 2026-07-29

### 고침

- 같은 밀리초에 찍힌 게이트 기록의 순서에 기대지 않는다
...
- **ADK 0.8.0 설계 대조 완결.** 설계 문서(`adk-prototype.md`·`adk-reference.md`)를 코드와
  전수 대조해 나온 불일치 19건을 0.8.2~0.8.13 에 걸쳐 전부 처리했다. 이 배포는 그 마무리다.

  구조가 바뀐 것:
  - 진입점이 `awl run`(CLI) + `/awl`(스킬)로 통일됐다. `/awl-loop` 은 명시 전용 레거시다.
  - **오케스트레이터가 은퇴했다.** plan/exec/review 역할 분할·`.tasks` 일감 큐·워처가
    사라지고 레인당 한 세션이 스펙부터 검증까지 관통한다. 서브에이전트는 설계가 허용하는
    셋(편향 회피·동시성·오염 방지)에만 뜬다.
  - **`~/.awl/engine` 사본이 사라졌다.** 설치된 npm 패키지가 유일한 엔진이라
    `npm i -g` 한 줄로 엔진이 올라간다. 실행 바이너리와 엔진이 어긋날 구조가 없어졌다.
  - 게이트 기록에 그 순간의 `loopMode` 가 각인된다. `auto` 가 실제로 전부 자동이 됐다.
  - 스펙에 `## Qualitative` 절이 생기고, EARS 검사가 모순·빠뜨린 예외까지 본다.

  곁다리로 잡은 실결함:
  - 배포 번들에 저작자 개인 절대경로가 기본값으로 박혀 있었다(모든 설치가 존재하지 않는
    남의 경로를 안내). 프로젝트 로컬 `.awl/feedback/` 으로 교체.
  - 링크로컬 IPv6(fe80::/10)에 zone index 없이 바인드해 리눅스에서 `awl port lease` 가
    EINVAL 로 죽었다.
  - CI 가 0.7.5 부터 매번 실패했다(1MB stdout 파이프 교착 · 위 EINVAL · node 18 에 없는
    플래그). 셋 다 고쳐 node 18·22 모두 복구.

  `adk-checklist.md` 는 폐기하지 않고 정정했다 — 자기 감사가 왜 못 잡았는지가 남아야 한다.

## [0.8.13] - 2026-07-29

### 고침

- node 18 에서 워커 스폰 테스트를 건너뛴다 — CI 매트릭스 복구
- 링크로컬 IPv6 바인드 EINVAL 로 CI 가 깨지던 것 수정
- 1MB stdout 교착으로 CI 가 9분씩 멈추던 것 수정

### 변경

- 문서를 0.9.0 구조에 맞추고 감사 목록을 정정 (설계 대조 2단계 #15·#16~18)

## [0.8.12] - 2026-07-29

### 고침

- auto 모드가 실제로 전부 자동이 되게 (설계 대조 2단계 #14)

## [0.8.11] - 2026-07-29

### 추가

- 검증 세 층과 정성 루브릭·반증 요구 (설계 대조 2단계 #12)

## [0.8.10] - 2026-07-29

### 추가

- 스펙 뼈대에 Qualitative 절, EARS 모순·누락예외 검사 추가 (설계 대조 2단계 #11·#13)

## [0.8.9] - 2026-07-29

### 변경

- 패키지명을 실제 배포명(agent-work-loop)으로 정정 (설계 대조 2단계 #10)

## [0.8.8] - 2026-07-29

### 변경

- 전역 학습층과 운영 명령을 설계에 명문화 (설계 대조 2단계 #8·#9)

## [0.8.7] - 2026-07-29

### 변경

- ~/.awl/engine 사본 제거 — npm 패키지가 유일한 엔진 (설계 대조 2단계 #7)

## [0.8.6] - 2026-07-29

### 고침

- 규칙 개수 상한 경고 제거 — 정리는 개수가 아니라 신호로 (설계 대조 2단계 #6)

## [0.8.5] - 2026-07-29

### 고침

- 게이트 2 의 이중 의미를 막고 워크아이템을 레거시로 내림 (설계 대조 2단계 #4·#5)

## [0.8.4] - 2026-07-29

### 변경

- 오케스트레이터·역할분할 은퇴 — 레인당 한 세션 (설계 대조 2단계 #3)

## [0.8.3] - 2026-07-29

### 추가

- /awl 을 정식 진입점으로 승격, awl-loop 은 명시 전용 레거시로 (설계 대조 2단계 #2)

## [0.8.2] - 2026-07-29

### 추가

- awl run — 요청을 여는 CLI 진입점 (설계 대조 2단계 #1)

## [0.8.1] - 2026-07-29

### 고침

- ADK 0.8.0 을 실제로 도달 가능하게 (설계 대조 감사 1단계)
...
- pre-push 훅을 기본 미설치로, --push-guard로 옵트인

### 변경

- AGENTS.md의 awl-loop 라우팅 블록을 커밋
...
- awl --version 출력에 버전별 설명 추가

### 고침

- 레인 워크트리는 등록된 프로젝트로 취급하지 않는다 [AC-01..AC-04]

## [0.6.43] - 2026-07-21

### 변경

- `awl-pipeline`·`awl-loop`가 스폰된 세션/서브에이전트의 완료를 사람에게 보고할 때(`@<이름> ...
  finished` 류 문구를 옮길 때) 맨 앞에 `date '+[%y%m%d / %H:%M:%S]'`로 얻은 24시간제 타임스탬프(예:
  `[260721 / 12:42:03]`)를 붙이도록 했다. 실제 시각은 반드시 `date` 명령으로 조회하고 추정해서 쓰지
  않는다. `awl-pipeline`의 "보고·응답 형식" 절에 적어 exec/review/plan 세 스킬에도 교차참조로 그대로
  적용된다. 코드 변경 없음 — `awl-pipeline/SKILL.md`·`awl-loop/reference.md` 문구만 갱신.
