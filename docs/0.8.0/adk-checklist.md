...
[o] 언제 아무 플래그도 안 주면, 서브에이전트를 하나도 안 띄워야 한다
    ...
    근거   2026-07-28 라운드1 WI-F4 로 고침. awl-loop/SKILL.md·reference.md — 완료조건 3개마다 트리거가 이제 --review 옵트인(기본 자가 검증: 조건↔검증↔커밋 대응·규칙위반·부존재탐지, 서브에이전트 없음). awl-pipeline 계열(exec+review 상시 2세션)은 의도적으로 안 건드림 — 레인 기반 독립 교차검증이 그 시스템의 핵심 아키텍처라 이 항목이 말하는 단일 loop 의 "기본 자가검증"과 다른 관심사(자가검증 자체가 코드로 강제되진 않음 — 아래 "?" 항목 참고)
