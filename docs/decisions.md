...
불확실한 지점을 만나면 "가정 / 근거 / 대안"을 남긴 뒤 진행한다. 시간순이고 번호는 다시
쓰지 않는다.

**결정은 고치지 않는다.** 바뀌면 새 결정을 써서 이전 것을 대체하고, 대체된 쪽에 그 사실을
한 줄로 표시한다. 무엇이 왜 뒤집혔는지가 남아야 같은 논의를 두 번 하지 않는다. 지금 유효한
결론만 보고 싶으면 README나 `awl --help`를 본다 — 이 파일은 판단의 이력이지 사용법이 아니다.

```
D-1 ~ D-46     0.1.x ~ 0.5.x   기반·CLI·격리 커밋·게이트·버전 경고
D-47 ~ D-54    0.6.x ~ 0.9.x   오케스트레이션 도입과 은퇴, ADK 대조, 문서 재편
```

`docs/specs/`·`docs/tickets/`·`docs/decisions/`(디렉토리)는 ADK 형식의 산출물이고, 이 파일은
그 이전부터 써온 단일 로그다. 앞으로 되돌리기 어려운 결정은 양쪽 중 작업 흐름에 맞는 쪽에
남기되, 여기 있는 이력은 지우지 않는다.
...
> **대체됨 → D-52.** 여기서 가정한 `~/.awl/lessons/`는 실제로는 `gotchas/`가 됐고(D-33),
> 기록(`records/`)은 0.8.x에서 프로젝트 로컬로 옮겼다. 저장 위치의 현재 결론은 D-52에 있다.
...
- **1차 리뷰(AC-01~03)**: 부정행위 없음. 사소한 결함 1건 — `narrative.kind` 검증이 `typeof data.kind === 'string'` 가드를 먼저 걸어서, `kind` 에 숫자 등 비문자열 truthy 값을 넣으면 required 체크도 enum 체크도 둘 다 우회해 무효한 kind 로 레코드가 만들어질 수 있었다. `kindMissing` 을 별도로 분리해 타입 불문으로 판정하도록 고치고, "kind 단독 누락"·"kind 가 숫자" 두 회귀 테스트를 추가했다. 그 외(스키마 필수 필드, 워크아이템 필터링, 하위호환, 테스트 성공/실패 양방향 커버리지)는 지적 없음.
- **2차 리뷰(AC-04~05)**: 부정행위 없음. **실제 결함 1건** — `awl metrics` 의 사람용 표(`renderMetrics`)가 `--json` 출력과 달리 `criteriaTotal` 열 자체를 빠뜨렸다(AC-04 스펙 위반). `renderMetrics` 가 `export` 도 안 되고 전용 테스트도 없어 아무도 못 잡았다 — `export` 하고 열을 추가하고, 전용 테스트(`renderMetrics — 사람용 표`)를 신설했다. **테스트 엄밀성 결함 1건** — "시간순 정렬" 테스트가 `WI-1`(이른 시각)/`WI-2`(늦은 시각) 조합을 썼는데 알파벳순과 시간순이 우연히 일치해, `sort()` 를 지워도 통과하는 무의미한 테스트였다 — 알파벳순과 시간순이 반대가 되는 `WI-Z`(이른 시각)/`WI-A`(늦은 시각) 조합으로 바꿔 진짜 회귀 테스트로 만들었다. 그 외(하위호환 0 처리, 깨진 JSON 스킵, 캐비트 양쪽 출력, `program.ts` 등록 패턴, 스킬 문서의 명령 예시/kind 4값 일치, `.claude/`↔`engine/skills/claude` 바이트 동일성)는 지적 없음. 최종 5/5 완료 조건, 360개 테스트. 게이트 1/2 는 이전 워크아이템들과 같은 자율 진행 근거(D-29 참고)로 자율 승인한다.
- **evolve 단계에서 자기 실증적으로 발견한 절차 누락**: `awl evolve --collect --workitem WI-P` 를 처음 돌렸을 때 `reviews:[]` 였다 — 리뷰를 실제로 두 번 돌렸는데도 판정을 `awl record review` 로 한 번도 기록하지 않았다. 스킬 문서(SKILL.md/AGENTS.awl.md) 의 "완료 조건 3개마다 리뷰" 절이 리뷰를 **돌리라**고만 하지 **기록하라**고는 안 했다 — WI-P 의 주제(계측)와 정확히 같은 종류의 누락이라 그 자리에서 고쳤다(스킬 문서에 한 줄 추가, 이번 두 리뷰 판정도 소급 기록). 이걸로 G-006 이 조용히 새는 필드/기록의 근본 패턴을 재확인하는 새 gotcha(G-018)를 만들었다.
- **최종 gotcha 계측(소급, 이 워크아이템 자체가 첫 실증 데이터)**: `gotcha-applied` 1건(G-013 — 매 완료조건마다 `npm run build` 직접 실행해 dist 최신 여부를 실제로 확인함), `gotcha-missed` 1건(G-006 — record.ts/evolve.ts 의 workitem/source 자동 채움 버그가 D-34/D-35 단계에서 3번째로 재발함, 사전에 gotcha 목록을 안 훑어보고 새 코드를 작성해서 발생). 최종 세대 스냅샷: `criteriaTotal:5, avgAttempts:0, blockedRatio:0, reviewRejects:2, proceduralErrors:0, gotchaApplied:1, gotchaMissed:1` — `awl metrics` 로 확인, WI-B~WI-O 는 전부 gotchaApplied/gotchaMissed 가 0(계측 이전 데이터, 하위호환 처리)이라 WI-P 가 이 지표의 사실상 첫 유효 데이터 포인트다.
- **후속 조치 (1) narrative 소급 기록 — 실제로 있었던 사례만**: 이 세션의 `~/.awl/records/` 를 실사 조사해 4개 kind 각각의 후보를 찾았다. **reviewer-caught 2건**(WI-H 의 `dist/cli.js` 미재빌드를 리뷰가 직접 `state.json`/`git diff` 대조로 잡아낸 사례, 이번 WI-P AC-04 의 `criteriaTotal` 사람용 출력 누락을 2차 리뷰가 잡아낸 사례) — 둘 다 `awl record narrative` 로 기록. **spike-prevented 1건**(WI-H 의 review 자료 조립 방식 3변형 실측 비교 스파이크 — 감으로 하나를 고르지 않고 신선한 서브에이전트 3개에 병렬 리뷰시켜 근거 있게 결정) — 기록. **gate-caught/blocked-discarded 는 소급 기록하지 않는다** — 실제로 전체 프로젝트 통틀어 `blocked` 타입 기록이 0건이고(이 세션에서 3회 실패로 코드를 버린 적이 한 번도 없다), 이 세션의 모든 게이트 1/2 는 자율 진행 근거로 승인만 됐을 뿐 실제로 뭔가를 발견해 막은 사례가 없다 — 없는 사례를 억지로 끼워 맞추지 않는다(스킬의 "해당하는 순간이 없었으면 억지로 기록하지 않는다" 원칙 그대로 적용). 앞으로 실제 사례가 생기면 그때 채운다.
- **후속 조치 (2) 같은 도메인 연속 진행 방법론**: `awl metrics` 로 세대를 비교할 때 신뢰도를 높이려면, 서로 다른 도메인의 워크아이템을 무작위로 비교하지 말고 **같은 도메인(예: CLI 명령 신설, 스키마 마이그레이션, 문서 동기화 등) 3~4개를 연속으로 진행한 뒤 그 안에서만 추세를 본다** — 난이도 통제가 안 된 이질적 워크아이템끼리는 `avgAttempts`/`blockedRatio` 차이가 실력 변화가 아니라 과제 난이도 차이일 수 있다(캐비트 문구가 경고하는 바로 그 위험). 이 세션 자체를 예로 들면 WI-F/WI-G/WI-H(전부 "검증/커밋 인프라 확장" 도메인)나 WI-O/WI-P(전부 "evolve 생태계 확장" 도메인)가 같은 도메인 연속 사례에 해당한다 — 사후적으로도 이 두 묶음 안에서는 세대 비교가 상대적으로 더 유효하다.
- **후속 조치 (3) 매 완료조건마다 gotcha 확인 습관화**: WI-P AC-05 로 스킬 문서에 이미 반영됨(위 참고) — 코드 변경은 이걸로 완료. 습관화 자체(실제로 매번 확인하는지)는 문서가 강제할 수 없는 영역이라 다음 워크아이템들에서 실제로 지켜지는지 계속 관찰이 필요하다.
- **후속 조치 (4) WI-B 를 기준선/대조군으로 취급**: WI-B 는 이 세션에서 `awl metrics` 로 조회 가능한 가장 오래된 세대이자(`criteriaTotal:11, avgAttempts:0.73, reviewRejects:2, proceduralErrors:4`), gotcha 계측 자체가 없던 시절의 데이터라 `gotchaApplied`/`gotchaMissed` 는 항상 0(계측 이전이라 "적용 안 함"이 아니라 "측정 안 함"임을 구분해야 한다 — `awl metrics` 의 캐비트와 별개로 이 구분 자체는 문서로만 남기고 코드로 강제하지 않는다, 과도한 엔지니어링 방지). 앞으로 세대 비교 시 WI-B 를 "이 프로젝트가 스스로를 검증하기 시작한 시점의 원점"으로 참고한다.

## D-36. `commit.ts` 자체검증 순환 참조 수정 — 0.4.x(WI-Q) 착수 전 선행 수정

- **발견 경위**: 0.4.x 스펙(사용자 전달)이 다른 프로젝트("editor-ux")의 3차 실사용에서 `awl commit`이 "자체 검증: 내가 쓴 파일만 커밋됨"을 출력하고도 완료조건 범위 밖 파일 14개(2개 파일에서 각각 122줄/73줄 삭제 포함)를 함께 흡수한 사례를 두 번째로 보고했다. 실제 코드(`isolatedCommit`)를 읽어 확인: `selfCheckOk`는 `committedFiles`(커밋 직후 `git show`로 읽음)와 `stagedFiles`(커밋 직전 `git diff --cached`로 읽음)를 비교하는데, 커밋은 스테이징된 내용 그대로 만들어지므로 이 비교는 항상 동어반복적으로 참이다(순환 참조) — 무엇이 처음에 잘못 스테이징됐는지는 절대 못 잡는다.
- **직접 재현 실험으로 확인한 것**: `--start` 이후 다른 커밋이 같은 브랜치 HEAD 에 얹히는 시나리오(다른 완료조건을 실수로 거기서 커밋했거나 동시 세션의 작업)를 실제로 재현했다. 신규 파일 추가는 우연히 `git apply --cached`의 index 충돌로 걸리지만(신규 스테이징 실패), **기존 파일 수정/삭제는 index 가 이미 드리프트 커밋 내용으로 reset 돼 있어 patch 컨텍스트가 안 맞아 이 역시 결국 hunk 충돌로 걸린다** — 즉 이 특정 시나리오(다른 "진짜 커밋"이 얹히는 경우)는 이미 우연히 막혀 있었지만, 원인이 "hunk 충돌"이라는 애매한 메시지로만 나와 진짜 이유(HEAD 이동)를 알 수 없었다. 실제 사고 리포트("14개 파일 흡수, 자체검증 통과 출력")와는 정확히 일치하지 않는다 — 가장 그럴듯한 실제 원인은 여러 완료조건에 걸친 작업을 하나로 몰아 커밋한 경우(각 완료조건마다 `commit --start`→`-m` 을 안 끊고 계속 편집)로 추정되지만, 다른 프로젝트의 실제 로그 없이 100% 확정할 수는 없다.
- **그래도 고친 것 — 두 가지, 둘 다 보수적**:
  1. **HEAD 드리프트 사전 감지**: `isolatedCommit`에 `expectedHead`(완료조건이 `--start`된 시점의 HEAD, `state.json`의 `criteria[].baseline`에 이미 저장돼 있던 값)를 새 선택 인자로 추가. diff/apply 를 시도하기 전에 현재 HEAD 와 대조해, 다르면 몇 개의 커밋이 어떤 내용으로 끼어들었는지 정확히 알려주고 커밋을 거부한다. `runCommit`이 이미 갖고 있던 `crit.baseline`을 그대로 넘기기만 하면 되어 새 CLI 옵션은 필요 없다. 기존 hunk-충돌 경로보다 먼저, 더 명확한 원인으로 걸린다.
  2. **오해를 부르는 메시지 정정**: "자체 검증: 내가 쓴 파일만 커밋됨." → "내부 검증: 스테이징한 내용 그대로 커밋됨."(동어반복 확인이라는 사실을 숨기지 않는다). 스테이징 파일이 5개(`STAGED_FILES_NOTICE_THRESHOLD`, 이 저장소의 완료조건이 실제로 보통 소수 파일만 건드리는 관행에서 고른 값)를 넘으면 개수를 눈에 띄게 알린다 — "맞다"는 판단은 awl 이 못 하니(완료조건의 의도된 범위를 모른다) 계산(개수)만 하고 대조는 에이전트/사람 몫으로 남긴다.
...
## D-38. WI-V 완료: 스킬 파이프라인에 [명료화] 단계 추가 (0.4.5, 사용자가 세션 도중 실시간 전달)

- **순서 예외**: 사용자가 원래 로드맵(WI-Q→R→S→T→U) 진행 도중 이 스펙을 실시간으로 전달했다. 스킬 자체를 바꾸는 메타 워크아이템이라 WI-R(그때 막 `work new` 만 하고 조사 전이었다)보다 먼저 처리해야 WI-R/S/T/U 부터 실제로 적용된다고 판단해, WI-R 을 손실 없이 일시정지(paused)하고 WI-V 로 먼저 처리했다. 버전 번호(0.4.5)는 사용자 지정 그대로 쓰되 실행 순서만 앞당겼다는 걸 여기 명시한다.
- **파이프라인 변경**: `[조사] → [설계] → [명료화] → [스파이크] → [완료 조건]` — [설계] 뒤, [스파이크] 앞. 핵심 구분은 "코드를 읽으면 답이 나오는 것(조사의 몫) vs 코드에 답이 없는 취향/방향 결정(되묻는다)". 되물을 게 없으면 건너뛴다(억지 질문 금지), 3개 넘으면 목표가 모호하다는 신호로 사람에게 알린다, 명료화(여러 번 대화)와 게이트1(한 번의 승인)은 별개. `awl record clarify`(questions 배열 필수, criteria.items 와 같은 패턴 — 내부 구조는 강제 안 함) 로 기록하고, 완료조건에 `clarifiedBy` 같은 자유 필드로 링크한다(스키마 변경 불필요, dependsOn 과 같은 관행).
- **절차 위반을 스스로 잡고 소급 문서화**: 완료 조건을 쓰기 전에 스킬 문서 편집을 먼저 시작해버려 절대 규칙 1(완료 조건 없이 구현 금지)을 위반했다 — 하지만 아직 아무것도 커밋하지 않은 상태였다(`git status` 로 확인). 편집 내용을 되돌리지 않고(스펙과 정확히 일치해 되돌리는 게 낭비), 그 자리에서 멈추고 완료 조건을 정식 작성한 뒤 **진짜** 게이트 1(`AskUserQuestion`)을 받아 이어갔다. `awl record decision` 으로 이 자기수정 자체를 기록했다 — WI-V(프로세스 신뢰성)의 취지상 조용히 넘기지 않는 게 맞다고 판단했다.
- **G-012 세 번째 재현, 즉시 복구**: AC-02 를 `--start` 하기 전에 이미 스킬 문서가 편집돼 있던 상태(위 절차 위반의 산물)라, 스냅샷이 그 편집을 흡수해 `git diff <snapshot>` 이 0줄로 나왔다. `awl state set` 으로 `criteria[AC-02].snapshot` 을 `baseline`(AC-01 종료 시점 HEAD)과 같은 값으로 되돌려 즉시 복구(87줄 diff 정상 회복 확인). `gotcha-missed` 로 소급 기록.
- **리뷰**: 부정행위 없음, 문제 없음(전부 확인 — 스키마 정확성, 두 문서의 다이어그램/본문 순서 일치, 6개 필수 내용 전부 포함, `.claude` 설치본↔`engine/skills/claude` md5 동일, 절대 규칙 2 문구 일관, 테스트 성공/실패 커버리지).
- **evolve**: `gotcha-missed` 1건(G-012 재현). 새 gotcha 1건(G-021 — 사용자가 매우 구체적인 스펙을 실시간으로 던지면 그 구체성 때문에 "이미 다 정해졌으니 절차를 생략해도 된다"는 유혹이 오히려 커진다 — 스펙의 구체성과 절차 생략 가능 여부는 무관함을 상기한다). 최종 2/2 완료 조건, 게이트 1/2 모두 실제 승인(`auto:false`).

## D-39. WI-R 완료: 워크아이템 등록을 강제한다 (0.4.1)

- **범위 2갈래**: AC-01 `awl record` 가 활성 워크아이템(데이터의 `workitem` 필드 > `--workitem` 플래그 > `state.json` 의 현재 워크아이템, 셋 다 없으면 거부), AC-02 스킬 문서에 절대 규칙 11("[조사] 전에 `awl work new`") + 비공식 관례(완료조건 ID 접두어로 워크아이템 흉내) 금지 경고.
- **[명료화] 단계를 실제로 처음 적용**: WI-V 가 막 만든 [명료화] 규칙을 이 워크아이템에 처음 적용해봤다 — 스펙을 다시 읽고 "코드로 답이 나오는 것(예: `state.workitem` 이 실제 필드명인지)" 과 "사람만 답할 수 있는 결정"을 구분한 결과, 후자가 하나도 없어 [명료화]를 건너뛰었다(스킬이 명시한 "되물을 게 없으면 건너뛴다" 규칙을 그대로 따름). 이 판단 자체를 `awl record audit` 에 명시했다.
- **핵심 설계 결정(G-022 로 남긴 재사용 가능한 교훈)**: `buildRecord`(순수 스키마 계층)를 `project` 처럼 전역 필수로 바꾸지 않고, `runRecord`(CLI 진입점)에서만 강제했다 — `buildRecord` 를 바꾸면 D-34 이후 쌓인 기존 테스트 40여개(모두 `workitem` 없는 `DEFAULTS` 사용)가 전부 깨지기 때문. `workitem` 이 "있으면 자동 태깅"(D-34)과 "없으면 거부"(WI-R)라는 서로 다른 두 목적을 겸하게 됐는데, 전자는 스키마 계층에, 후자는 실행 진입점에 각각 둬서 파급을 분리했다.
- **조사에서 발견: 요구사항 4번(`awl records --workitem` 필터)은 이미 구현·테스트돼 있었다** — 새 완료조건을 만들지 않고 조사 기록으로만 남겼다(과잉 구현 방지).
- **리뷰가 사소한 지적 2건 발견**: (1) `resolveProjectRoot()` 가 `null` 인 완전히 프로젝트 밖 실행 상황에서 "활성 워크아이템이 없습니다" 메시지가 진짜 원인(프로젝트 미초기화)을 안 알려줌 — `projectRoot` 유무에 따라 힌트 문구를 추가해 즉시 수정. (2) 스킬 문서가 workitem 의 3가지 소스 중 2개(state.json/--workitem)만 언급하고 데이터 안 `workitem` 필드를 안 다룸 — 두 문서 모두 3가지 다 명시하도록 정정. 부정행위 없음.
...
## D-41. WI-T 완료: 완료 조건의 질을 검사한다 (0.4.3, 사용자가 "가장 중요"로 표시)

- **배경**: 이전 실사용 세션에서 완료 조건 9개가 전부 1차 시도로 통과했는데, 이는 실패 처리 능력 검증이 아니라 완료 조건을 사소하게 썼다는 신호였다. 두 가지 근본 원인 — (1) 조사에서 찾은 어려운 문제를 사람 승인 없이 조용히 범위 밖으로 뺌, (2) 완료 조건에 "저위험 건 수정" 같은 질적 표현이 남아 구현 도중 재해석됨.
- **범위 5갈래(최초 계획)**: AC-01 완료 조건에 금지된 질적 표현 5개(저위험/주요한/적절한/가능한 만큼/필요시) 거부, AC-02(핵심) `computeCoverage` 순수 함수 신설 — audit findings 의 id 와 완료조건의 `addresses` 를 대조해 배제 목록을 계산하고, 게이트 1 기록 시 배제가 있는데 `presentedExclusions` 로 명시 제시하지 않으면 거부, AC-03 게이트 2 기록 시 "완료 조건 전부 1차 통과+막힘 0건"이면 커버리지 수치와 함께 "충분히 야심찼는가" 안내(거부 아님), AC-04 evolve/metrics 에 `coverage`(auditFindingsTotal/addressed/excluded/excludedApprovedByHuman) 계측 추가, AC-05 세 스킬 문서 갱신(절대 규칙 12 신설: "조사에서 찾은 문제를 조용히 범위 밖으로 빼지 않는다").
- **리뷰(rev_07187aa7bfa3ad93d7)가 핵심 안전장치 자체의 구조적 결함을 발견**(`verifyPassedBefore:true` 인 채로 `findings` 4건 — `reviewer-caught` narrative 로 별도 기록): (1) **high** — `computeCoverage` 가 `state.criteria[].addresses` 만 읽는데, 스킬 문서의 `awl state set` 예시가 `addresses` 를 안 옮겨서(그 필드는 `awl record criteria` 예시에만 있었다) 문서를 그대로 따르면 정상적으로 다뤄진 발견도 게이트 1에서 배제로 오판됐다 — 리뷰어가 실제 CLI 로 문서 예시를 그대로 실행해 재현까지 했다. AC-06 으로 편입해 `computeCoverage` 에 `criteriaRecords`(append-only `criteria` 레코드) 인자를 추가, `state.criteria` 에 `addresses` 가 없는 완료조건만 레코드에서 보완하도록 고치고(state 값이 있으면 그게 우선), 세 스킬 문서의 `state set` 예시에도 `addresses` 를 채워 문서 자체의 함정도 없앴다. (2)(3)(4) low — 금지어 검사의 부분 문자열 오탐 가능성("부적절한"⊃"적절한" 등), `presentedExclusions` 순수 문자열 배열 케이스 테스트 부재, `excludedApprovedByHuman` 의 `auto` 비-boolean 케이스 테스트 부재 — AC-07 로 편입해 한글 음절 경계 판정(`includesBannedWord`)과 누락된 테스트 2건을 추가했다. 부정행위는 없음.
- **자기지시적 사례 두 번**: (1) AC-07 의 완료 조건 텍스트를 처음 쓸 때 "부적절한"/"필요시간" 예시를 그대로 적었다가, 방금 만든 AC-01 의 금지어 필터 자신에게 거부당했다 — 좋은 자기증명이라 리뷰 rev id 참조로 다시 써서 우회했다(둘러대지 않고 그대로 겪은 사실을 여기 남긴다). (2) 이 워크아이템 자체도 완료 조건 7개(AC-01~05 최초 계획 + 리뷰 편입 AC-06/07) 전부 1차 통과·막힘 0건이라, AC-03 이 만든 "충분히 야심찼습니까" 안내가 게이트 2 기록 시 그대로 떴다 — 다만 5개로 시작해 리뷰가 실질적 구조 결함을 2개 더 찾아 7개가 됐으므로, 이 경우는 "완료조건을 사소하게 썼다"기보다 "리뷰가 실제로 커버리지를 넓혔다"는 사례로 보는 게 맞다.
- **소소한 관찰(코드 수정 없음)**: [명료화] 단계에서 "되물을 게 없다"는 판단을 `awl record audit` 의 finding(`CL-01`)으로 남겼는데, 이건 코드 결함이 아니라 프로세스 판단이라 어떤 완료조건도 `addresses` 로 연결하지 않았다. 게이트 1 시점엔 아직 AC-02(배제 강제 코드) 자체가 구현되기 전이라(부트스트랩 순서상 당연함) 걸러지지 않았고, 그 결과 evolve 의 커버리지 지표가 8건 중 7건(87.5%)으로 집계됐다 — 버그가 아니라 "발견 종류에 안 맞는 곳에 적었다"는 기록 습관의 사소한 어긋남이다. 다음엔 이런 순수 판단 메모는 `awl record audit` 대신 `awl record decision` 에 남긴다.
- **evolve**: 새 gotcha 2건 — G-025(여러 CLI 명령에 걸쳐 값이 전파돼야 하는 기능은 문서 예시를 실제로 그대로 실행해 재현 검증), G-026(금칙어 부분 문자열 매칭은 형태소 결합 언어에서 반의어/파생어 오탐 위험 — 문자 범주 경계 판정 필요). `blocked` 없음. 최종 7/7 완료 조건(AC-01~05 최초 계획, AC-06/07 리뷰 지적 편입), 게이트 1/2 모두 실제 승인(`auto:false`). 게이트 2 이후 사용자가 명시적으로 "이제 묻지 말고 자체 판단하에 쭉 진행"을 지시함 — 이후 워크아이템은 게이트를 `auto:true` 로 기록한다.

## D-42. WI-U 완료: 기록 상세도를 diff 크기에 맞춘다 (0.4.4)

- **배경**: "완료조건마다 `awl record attempt`로 what/why/how를 산문으로 쓰는" 단계가 `recordHistory:true` 한 줄 추가 수준의 변경에는 무게가 안 맞는다는 지적 — 토큰 낭비이기도 했다.
- **범위 3갈래(AC-01/02 는 서로 결합돼 같은 커밋으로 구현됨)**: AC-01 `awl record attempt` 가 diff 크기를 스스로 측정(`result:"passed"` 는 방금 만든 커밋을 `git show --numstat HEAD`, `result:"failed"` 는 작업트리를 `git diff --numstat HEAD`) 해 `detailTierFor()`(순수 함수, minimal/brief/detailed 3단계)로 상세도를 정하고 stderr 로 안내한다. AC-02 `SCHEMAS.attempt` 를 `{what, result}` 로 줄이고 `why`/`how`/`alternatives` 를 diffTier 조건부 필수로 바꾼다 — **핵심 요구사항: `result:"failed"` 는 diff 크기와 무관하게 항상 `why`/`how` 전부 요구**(정보 손실 방지, 실패가 gotcha 의 원천). diffTier 가 없으면(git 측정 실패 등) 안전한 쪽(전체 상세 요구)으로 기운다. AC-03 세 스킬 문서 갱신.
- **설계 결정**: diff 크기 측정을 `state.currentFocus`/`resolveBlockedBaseline` 경로에 태우지 않기로 했다 — 조사([조사] audit F-02)에서 그 필드가 스킬 문서 어디에도 설정 지시가 없어 실사용에서 거의 항상 비어있음을 확인했다(blocked 의 baseline 자동 첨부 기능도 사실상 죽어있을 가능성). 대신 "`awl commit`이 정확히 커밋 하나를 만들고 그 직후 `record attempt`가 온다"는 스킬의 고정된 순서를 이용해 `git show HEAD`로 직전 커밋을 재는 쪽을 택했다 — 별도 상태 필드 없이 항상 정확하다.
- **자기지시적 사례**: 이 워크아이템 자체를 구현하는 도중 새 기능이 구현자 자신의 `awl record attempt` 호출을 두 번 거부했다(282줄/2파일 변경에 `alternatives` 누락, 12줄/2파일 변경에 `why`/`how` 누락) — 만든 지 몇 분 안 된 기능이 스스로에게 바로 적용된 사례.
- **리뷰(rev_175abe6265e2b30ef6)는 낮은 심각도 3건만 발견, 전부 비차단으로 명시적으로 판정**: (1) low — "방금 만든 커밋" 전제가 코드로 검증되진 않는다(동시 커밋이 끼면 오측정 가능, 크래시는 없음) — 이미 구현자가 audit/코드 주석으로 트레이드오프를 문서화해둔 것이라 추가 조치 없음. (2) low — AC-01/AC-02 를 한 커밋으로 합친 "서로 없이는 테스트 불가능" 근거가 다소 과장됐다(실제로는 diffTier 명시 테스트로 AC-02 단독 테스트가 가능했음) — 판단 자체는 여전히 합리적이라 재작업 없음. (3) info — 바이너리 파일 numstat 케이스와 brief tier 통합 테스트가 없다(순수함수 테스트로는 커버됨, 리뷰어가 직접 재현해 크래시 없음 확인). 부정행위 없음. **이번엔 세 지적 모두 리뷰어가 명시적으로 "차단 아님"이라 판정해 새 완료 조건으로 편입하지 않았다** — WI-S/WI-T 와 달리 낮은 심각도 지적을 전부 새 AC 로 만들지 않은 첫 사례(단위를 작게 유지하라는 사용자 지시와 일치하는 판단).
- **evolve**: 새 gotcha 없음(리뷰가 재사용 가능한 새 교훈을 낼 만한 구조적 결함을 못 찾음). `blocked` 없음. 최종 3/3 완료 조건, 게이트 1/2 모두 `auto:true`(사용자가 명시적으로 지시한 자율 진행 — WI-T D-41 참고).

## D-43. WI-W 완료: narrative 에 tool-failed 종류 추가 + 0.4.x 발표 서사 소급 기록

- **배경**: 0.4.x 스펙 부록이 "소급 기록하라. 이번 세션이 가장 좋은 재료를 냈다"며 서사 5건(gate-caught 2, spike-prevented 1, reviewer-caught 1, 그리고 새 종류 `tool-failed` 1)을 표로 제시했다. 마지막 항목(도구 자체 결함)은 "발표에서 숨기지 마라 — 우리 도구가 이렇게 틀렸고, 이렇게 발견했고, 이렇게 고쳤다가 완벽합니다보다 훨씬 설득력 있다"는 명시적 지시가 붙어 있었다.
- **범위 2갈래**: AC-01 `NARRATIVE_KINDS` 를 4값에서 5값(`tool-failed` 추가)으로 확장, 세 스킬 문서 narrative 섹션 갱신. AC-02 실제 근거를 찾아 서사 5건을 `project:maxflow`(0.4.x 스펙이 나온 실사용 대상 프로젝트, D-36 이 "editor-ux"로 지칭한 것과 같은 프로젝트 — `.awl config.json` 상의 실제 project 필드명은 maxflow)로 `awl record narrative` 소급 기록.
- **소급 기록의 근거를 실제 과거 레코드에서 찾음(지어내지 않음)**: `~/.awl/records/`를 직접 조회해 project:maxflow 의 과거 `audit`/`attempt` 기록에서 4건의 정확한 원본을 찾았다 — 단일 audit 레코드(`rec_1ec6e72da646431fe8`, findings 19개짜리)의 finding[7](undo/redo recordHistory 0건), finding[11](chrome-lint chromeDir 스캔범위 누락), finding[5](ItemSelector 다중선택 데이터모델 제약), 그리고 별도 attempt 레코드(`rec_e55609527cbf110d2a`)의 `why` 필드(AC-C1 주 진입점 누락 리뷰 지적). 5번째(tool-failed)는 project:maxflow 에도, docs/decisions.md 어디에도 `awl record` 로 남은 적이 없고 D-36 의 "발견 경위" 산문에만 서술돼 있었다 — 소급 기록이 정확히 이 갭을 메운다.
- **project:maxflow 에는 review 타입 레코드가 단 1건도 없다**(WI-S 가 리뷰 스키마를 만들기 전 시기라 애초에 존재할 수 없었다) — reviewer-caught 서사를 `reviewId`로 못 엮고 `attempt.why`에 남은 산문 인용으로만 근거를 댔다. WI-S 가 막으려던 "리뷰가 실사고를 잡아도 데이터로 안 남는" 문제의 실제 사례.
- **AC-02(순수 데이터 기록) 진행 중 WI-U 리뷰가 예견한 갭이 즉시 재현됨**: `awl commit` 을 만들 코드 변경이 없는 완료조건에서 `awl record attempt` 를 호출하자, "방금 만든 커밋을 잰다"는 diff 측정 로직(D-42)이 AC-01(직전의 진짜 코드 커밋)의 diff 를 엉뚱하게 재측정해 "25줄/4파일, alternatives 필요"로 오판했다 — WI-U 리뷰 finding #1이 "동시 커밋이 끼면 오측정 가능"이라 지적했던 것과 정확히 같은 클래스(이번엔 "동시 커밋"이 아니라 "이번 완료조건엔 새 커밋이 아예 없음"이라는 변주). `diffTier:"minimal"` 을 데이터에 직접 명시해(스킬 문서가 이미 안내하는 하위호환 경로) 우회했다 — 코드 수정은 하지 않았다(리뷰가 이미 비차단으로 판정한 알려진 한계라 별도 완료조건으로 만들지 않음).
- **evolve**: 새 gotcha 없음. `blocked` 없음. 최종 2/2 완료 조건(3개 미만이라 리뷰 생략 — 스킬 규칙 그대로), 게이트 1/2 모두 `auto:true`. **이 워크아이템으로 0.4.x 로드맵(WI-Q~U, D-40~43) 이 전부 닫혔다.**

## D-44. WI-X 완료: 버전 불일치 경고 (0.5.0)

> **부분 대체됨 → D-51.** 여기서 감지하기로 한 4쌍 중 "설치된 엔진 vs 실행 바이너리"는
> 0.8.7에서 엔진 사본을 없애면서 **어긋날 구조 자체가 사라져** 제거했다. 나머지 쌍과
> `awl version-check`·`skills-version.json` 설계는 그대로 유효하다.

- **배경**: 0.5.x 스펙(사용자 전달)이 `awl --version`을 쳐도 버전이 어긋나면 경고가 없어 "고쳤는데 왜 안 바뀌지"가 반복됐다고 지적. 어긋날 수 있는 버전 4쌍(package.json vs engine/version.json, 설치된 엔진 vs 실행 바이너리, 프로젝트 config vs 전역 엔진, 설치된 스킬 vs 엔진)을 감지해 노란색+[!] 마커로 경고해야 한다는 요구. **원래 스펙 ID는 WI-W였으나 같은 세션에서 이미 다른 워크아이템(narrative 소급 기록)이 WI-W를 선점해 WI-X로 재배정했다(버전 번호 0.5.0은 스펙 그대로).**
- **조사에서 발견: 인프라가 이미 상당히 있었다** — `tty.ts`의 `makeColors()`에 `yellow`가 이미 있었고, `program.ts`의 `versionString()`이 이미 pair #2(바이너리 vs 엔진)를 텍스트로 경고하고 있었고, `doctor.ts`가 이미 pair #3(status:warn)과 스킬 설치 여부를 검사하고 있었다. 반면 `awl update` 명령은 `init.ts` 코드 주석("engine 갱신은 update 의 몫")에서만 언급되고 실제로 존재하지 않았다 — 이 갭을 메우지 않으면 모든 경고 힌트가 존재하지 않는 명령을 가리키게 되므로, [명료화] 없이 조사만으로(이미 코드가 그 설계를 명시해뒀으므로) AC-05로 새로 만들기로 결정했다.
- **범위 7갈래**: AC-01 `awl init`이 스킬 설치 시 `.awl/skills-version.json`에 설치 시점 엔진 버전을 기록(pair #4의 유일한 근거, 이전엔 계산할 방법 자체가 없었다). AC-02 `src/core/versions.ts`에 `checkVersions(inputs)` 순수 함수 신설 — 4쌍(스킬은 claude/codex 둘로 나뉘어 실질 5개 kind)을 계산, 값이 null이면 그 쌍은 건너뜀(크래시 없음). AC-03 `awl version-check` 명령 신설(`--json`). AC-04 `awl --version`/`awl doctor`가 노란색+`[!]`(경고)/`[!!]`(오류) 마커로 구분(예전엔 색 없으면 warn/fail이 똑같이 `-> hint`로 안 구분됐다) — **`versionString()`의 "awl init을 다시 실행하세요" 안내가 오답이었음을 발견해 "awl update로 엔진을 갱신하세요"로 정정**(이 쌍은 프로젝트 재설정이 아니라 엔진 자체 갱신이 필요). AC-05 `awl update` 명령 신설(엔진 재설치). AC-06 세 스킬 문서에 파이프라인 시작(워킹트리 확인보다도 먼저)에 `awl version-check` 호출 지시 추가. AC-07 리뷰 지적(medium) 반영 — `writeSkillsVersionStamp`의 병합 보존/손상파일 복구를 직접 증명하는 테스트 추가(코드는 이미 정확했음을 확인).
- **설계 결정**: 설치된 스킬 버전을 SKILL.md 프론트매터가 아니라 `.awl/skills-version.json` 사이드카 파일에 기록하기로 했다 — 에이전트가 읽는 지침 문서를 awl 내부 메타데이터로 오염시키지 않고, `release.mjs`가 그 문서까지 매번 갱신하게 만들지 않기 위해서다(`.awl/config.json`/`.awl/state.json`과 같은 자리에 두는 이 코드베이스의 기존 관례를 따름).
- **리뷰 2회(완료 조건 7개, 3개마다 나눠서)**: 1차(rev_d5925bd781b880c60a, AC-01~03) — medium 1건(`writeSkillsVersionStamp` 병합/손상파일 로직이 코드는 정확하나 직접 테스트 없음, AC-07로 편입해 수정), low 1건(스킬 kind 명명이 형식적, 조치 없음). 2차(rev_136e212845036f482c, AC-04~07) — low 1건(빌드 무결성 pair의 doctor 통합 지점 전용 테스트 없음), info 1건(`awl update`의 겹쳐쓰기가 고아 파일을 정리 안 함 — `scaffoldGlobal()` 최초 설치 로직과 동일 패턴을 상속한 것이라 새 결함 아님). **WI-U와 같은 패턴으로, 이번에도 low/info 지적은 새 완료 조건으로 만들지 않고 판정만 기록했다** — 사용자의 "단위를 작게 유지" 지시와 일치. 두 리뷰 모두 부정행위 없음.
- **자기지시적 사례**: WI-U에서 만든 diffTier 재측정 로직의 알려진 한계("동시 커밋이 끼면 오측정")가 이번엔 다른 변주로 재현됐다 — AC-02(순수 데이터 없음, 코드 커밋 없음)에서 `awl record attempt`를 호출하자 직전 AC-01의 진짜 코드 커밋을 엉뚱하게 재측정해 "25줄/4파일" 오판이 났다. `diffTier:"minimal"` 직접 명시로 우회(이미 문서화된 하위호환 경로), 코드 수정 없음.
- **evolve**: 새 gotcha 1건 — G-027(에러/경고 메시지의 해결 명령 안내가 실제로 그 문제를 해결하는지 코드로 확인한다 — 안내 문구는 진짜 해결 명령이 아직 없던 시절에 쓰였을 수 있고, 나중에 명령이 추가돼도 자동으로 안 바뀐다). `blocked` 없음. 최종 7/7 완료 조건(AC-01~06 최초 계획, AC-07 리뷰 지적 편입), 게이트 1/2 모두 `auto:true`(사용자 지시에 따른 자율 진행).

## D-45. WI-Y 완료: CLI 인터랙션 강화 — 방향키 선택 (0.5.1)

- **배경**: 0.5.x 스펙(사용자 전달) 두 번째 항목. `awl init`의 언어/스킬 선택이 번호 입력뿐이라 상호작용이 무겁다는 지적 — raw-mode 가능하면 방향키+Space 선택을, 안 되면 기존 번호 입력을 그대로 쓰는 폴백 설계 요구. 원래 스펙 ID는 WI-X였으나 같은 세션에서 버전 불일치 경고 워크아이템이 이미 WI-X를 선점해 WI-Y로 재배정했다(버전 번호 0.5.1은 스펙 그대로). 사용자가 "승인 게이트 없이, 다 내 의도가 담긴거니까, 그대로 진행해줘"라고 명시해 게이트 1/2 모두 `auto:true`로 진행했다.
- **범위 8갈래(AC-06/07/08은 리뷰 지적 편입)**: AC-01 `tty.ts`에 `computeRawModeCapable(stdinIsTTY, hasSetRawMode, ci)` 순수 함수와 `rawModeCapable()` 래퍼 신설 — `Caps`(출력 렌더링 능력, stdout 기준)와는 축이 달라(입력 처리 능력, stdin 기준) 별도 타입으로 뒀다. AC-02 `select.ts` 신설 — `advanceSelect`(순수 상태전이)/`renderSelectOptions`(순수 렌더), 단일/다중선택을 같은 상태 타입으로. AC-03 `runInteractiveSelect`(raw-mode I/O 루프) — 애초에 "터미널이 필요해 수동 시연만 가능"으로 계획했으나, `process.stdin.setRawMode`가 이 저장소 테스트 환경에서 `undefined`라 `vi.spyOn`이 못 거는 것을 직접 대입/복원으로 우회하고 `once('data', cb)` 콜백을 stdin 모킹으로 캡처·수동 호출하는 방식으로 21개 자동 테스트를 만들어 계획보다 커버리지를 넓혔다. AC-04 `init.ts`의 `interactiveInputs`에서 언어 단일선택/스킬 다중선택을 `rawModeCapable()`로 분기(`selectSingle`/`selectMulti` 신설), `config.ts`는 의도적으로 무변경(같은 `promptNumber`를 계속 재사용). AC-05 CI=true+stdin파이프에서 `init --yes`가 크래시 없이 끝남을 빌드된 `dist/cli.js` 실제 spawn으로 회귀 고정. AC-06 `docs/decisions.md` Windows 리스크 목록에 raw-mode 이스케이프 시퀀스 항목(7) 추가, D-13에 "단위테스트 불가능하다"던 옛 근거가 이번 워크아이템으로 반증됐다는 업데이트 노트. AC-07(리뷰 편입) `select.test.ts`의 `mockStdin()`이 `vi.spyOn(process.stdin, 'once')`를 콜스루로 둬 실제 `EventEmitter`에 `data` 리스너가 등록되고 한 번도 해제 안 되는 누수를 `mockImplementation`으로 근본 차단, `listenerCount` 회귀 단언 추가. AC-08(리뷰 편입) AC-05 테스트가 이름/주석과 달리 실제로는 raw-mode 분기를 하나도 안 탄다는 지적을 테스트명/주석 정정 + `it.each`로 해소, `selectSingle`/`selectMulti`의 `useRawMode:true` 배선을 직접 단위테스트로 신규 커버.
- **설계 결정**: raw-mode 선택이 가능해져도 번호 입력을 없애지 않고 폴백으로 유지했다(D-13 결정 존속) — Windows conhost 등 raw-mode가 실제로 깨지는 환경(macOS에서는 실측 불가)에 대한 안전망. `selectSingle`/`selectMulti`는 `useRawMode`를 주입받는 순수 배선 함수로 두고 내부에서 `rawModeCapable()`을 직접 안 불러, 실제 터미널 없이도 두 경로(방향키/번호) 모두 자동 테스트할 수 있게 했다.
- **자기지시적 사례**: (1) AC-05와 AC-06 둘 다에서 파일을 먼저 편집하고 나서 `awl commit --start`를 호출해, 스냅샷이 편집 내용을 이미 포함해버려 격리 커밋이 빈 diff로 거부되는 절차적 실수가 반복됐다 — 이미 존재하던 gotcha(G-012, WI-U에서 만들어짐)가 정확히 이 패턴을 예언하고 있었는데도 구현 시작 전 `awl gotchas`를 훑지 않아 두 번 다 놓쳤다(`gotcha-missed`로 소급 기록). 둘 다 `git checkout`으로 원복 후 순서를 바로잡아 복구했다(diff 내용 자체는 안 잃음). (2) 1차 리뷰(rev_a94f24e6be537bff14, AC-01~03 대상)가 "리뷰 완료를 기다리지 않고 AC-04 커밋, AC-05까지 진행했다"는 프로세스 지적을 냈다 — 리뷰 대상 파일 자체는 오염되지 않았지만(diff로 확인), 완료 조건 3개마다 리뷰를 보내고 그 결과를 기다리는 체크포인트 규율이 안 지켜진 사례로 기록. 이후 2차 리뷰(AC-04~07)는 결과를 기다린 뒤에야 게이트 2로 진행했다.
- **리뷰 2회(완료 조건 8개, 3개 이상마다 나눠서)**: 1차(rev_a94f24e6be537bff14, AC-01~03) — medium 1건(`select.test.ts`의 리스너 누수, AC-07로 편입), info 1건(위 프로세스 지적, 조치는 규율 준수로 갈음·새 완료조건 안 만듦). 2차(rev_b9f3bb4b93ede055f5, AC-04~07) — medium 1건(AC-05 테스트가 실제로 raw-mode 분기를 안 탄다는 지적, 직접 재현 확인됨) + low 1건(`selectSingle`/`selectMulti`의 `useRawMode:true` 배선 직접 커버리지 없음) — 두 건 모두 AC-08로 편입. 두 리뷰 모두 부정행위 없음, `verifyPassedBefore` 실제 재검증까지 확인.
- **환경 노트(코드 결함 아님)**: 이 세션 셸의 PATH에 `node_modules/.bin`이 없어 `awl verify`가 `biome`/`vitest`를 `command_not_found`로 보고한 적이 여러 번 있었다 — `rtk` 셸 후킹과 무관한, 이 세션 고유의 PATH 문제. `PATH="$PWD/node_modules/.bin:$PATH"`로 보정하면 매번 정상 통과했다(리뷰어도 독립적으로 재확인).
- **evolve**: 새 gotcha 1건 — G-028(회귀 테스트를 짤 때 이름/주석이 주장하는 검증 대상이 실제로 그 코드 경로를 타는지 직접 추적해 확인한다 — 특정 플래그가 다른 로직을 구조적으로 완전히 우회하면 주변 환경변수를 바꿔도 그 로직은 한 번도 실행 안 되는데 테스트는 계속 통과해 커버리지가 있다는 착각만 남긴다). 기존 gotcha 1건 놓침 — G-012(baseline 갱신 전 편집 순서, 두 번 재현). `blocked` 없음. 최종 8/8 완료 조건(AC-01~06 최초 계획, AC-07/08 리뷰 지적 편입), 게이트 1/2 모두 `auto:true`.

## D-46. 대형 모노레포에서 `awl lane new`/`awl work new --worktree` 타임아웃 버그 수정 + 워크트리 생성 리포팅

- **배경**: 15만+ 파일 모노레포에서 `awl lane new <name>`이 100% 재현되는 실패를 냈다 — `Updating files: 100% (152312/152312), done.` 직후 `fatal: Could not write new index file.`로 죽는다. 표면 메시지는 디스크/인덱스 문제처럼 보이지만 근본 원인은 다르다. `work.ts`의 `createGitWorktree`가 `git worktree add`를 `timeoutMs: 30_000`(30초)으로 호출하는데, 실측 체크아웃(디스크 88GB 여유·경합 없음 기준)은 40.177초가 걸린다. `runner.ts`의 `run()`이 타임아웃 시 `child.kill('SIGTERM')`을 보내고, git은 인덱스 파일을 쓰다 만 채 죽는다 — 30초가 리포 크기 대비 너무 짧은 게 원인이지 디스크 문제가 아니다. `lane.ts`는 이 경로를 `work new --worktree --isolated`로 그대로 재사용하므로 `awl work new --worktree`도 동일하게 영향받고 동일 수정의 혜택을 받는다.
- **결정**: (1) 기본 타임아웃을 30초 → 180초(4.5배 여유)로 올리고, `AWL_GIT_WORKTREE_TIMEOUT_MS`(ms) 환경변수로 완전히 오버라이드할 수 있게 했다(`resolveGitWorktreeTimeoutMs`, `AWL_HOME`/`AWL_ALLOW_PUSH`와 같은 env override 선례를 따름). (2) 타임아웃으로 실패하면(`RunResult.timedOut`) git의 원문 stderr 뒤에 실제 사용된 초 수와 `AWL_GIT_WORKTREE_TIMEOUT_MS` 안내를 덧붙인다(`buildWorktreeError`, 순수 함수) — 원문 메시지("Could not write new index file")만으로는 원인이 타임아웃인지 디스크 문제인지 구분이 안 됐던 이번 버그의 오진 경로를 다시 열지 않기 위해서다. (3) `run()`이 이미 정확히 그 git 프로세스만 재는 `durationMs`를 재사용하고(재측정 없음), `fs.statfsSync`로 잰 디스크 여유공간 델타(10진 GB, `df -H` 관례)를 성공/실패 무관하게 한 줄로 출력한다(`formatWorktreeReport`). `readDiskAvailable`은 `typeof fs.statfsSync !== 'function'`이면(구버전 Node) `null`로 낮아지는 best-effort다 — `package.json`의 `engines.node`는 `">=18"`인데 `statfsSync`는 v18.15.0부터라 이 갭이 실재한다.
- **파일 수 비례 동적 타임아웃 산정을 택하지 않은 이유**: 파일 수는 체크아웃 시간과 상관관계가 약하고(대형 바이너리 vs 소형 텍스트 파일 다수), 사전 조회(`git ls-files | wc -l`) 자체가 체크아웃 전에 실행돼야 하는 새 실패 지점이 된다. env override가 더 단순하고 사용자에게 완전한 제어권을 준다.
- **리포팅 위치 트레이드오프**: 소요시간·디스크 델타 출력은 `lane.ts`가 아니라 `work.ts`의 `createGitWorktree`/`runWorkNew` 안에 둔다. `lane.ts`에서 별도로 감싸 측정하면 스킬 재설치·verify baseline 같은 무관한 I/O가 디스크 델타에 섞여 "git worktree add 단계의 실제 소요시간"이라는 요구를 못 지킨다. `lane.ts`는 `work new --worktree --isolated` 원시경로를 그대로 재사용하는 얇은 래퍼라(기존 주석에 명시된 원칙) 소스 변경 없이 `awl lane new` 출력에 자동으로 나타난다 — 다만 화면상 위치가 `runWorkNew`가 먼저 찍는 "워크아이템 생성" 블록에 붙고, `lane.ts`가 나중에 찍는 "레인 준비" 블록보다 앞선다(정보 자체는 같은 출력 스트림에 동일하게 포함).
- **제외 항목**: 트래킹 파일 수(`git ls-files`) 카운트는 뺐다 — 대형 리포에서 그 자체가 지연을 더할 수 있어, 타임아웃 문제를 고치는 경로에 새 지연 요소를 추가하지 않기 위해서다. `AWL_GIT_WORKTREE_TIMEOUT_MS`를 `removeGitWorktree`/`removeWorktreeDir`(`git worktree remove`)에도 적용하는 것도 같은 클래스의 잠재 결함이지만 이번 버그의 재현·완료조건 범위 밖이라 후속 후보로만 남긴다.

## D-47. 오케스트레이션 파이프라인 도입 (0.6.x~0.7.x)

> **대체됨 → D-49.** 이 구조는 0.8.4에서 은퇴했다. 왜 만들었고 왜 접었는지가 함께 남아야
> 같은 설계를 다시 시도하지 않는다.

- **배경**: 워크아이템 하나를 한 세션이 관통하는 `awl-loop`만으로는 무관한 작업 여럿을 같은
  날 돌릴 수 없었다. 세션이 하나라 순차 처리뿐이고, 사람이 매번 다음 일감을 물려줘야 했다.
- **결정**: 레인(격리 워크트리)마다 plan·exec·review 세 역할 세션을 두고, `.tasks/` 파일 큐로
  일감을 넘기고, 파일 워처로 유휴 세션을 깨우는 구조를 만들었다. plan 세션이 오케스트레이터가
  되어 나머지를 스폰했다. 게이트 밀도(`gate-high`/`gate-medium`/`gate-low`)로 사람 개입량을
  조절했다.
- **왜 역할을 나눴나**: 사람 팀의 기획·구현·리뷰 분업을 그대로 옮겼다. 리뷰를 만든 사람이
  하면 편향이 생긴다는 판단은 지금도 유효하다(D-49 참고).
- **실제로 겪은 것**: 무인으로 오래 도는 건 됐다. 대신 세 세션이 동시에 사람에게 물어 출력이
  섞였고, 오케스트레이터 자체가 토큰을 먹었고, 핸드오프에서 맥락이 샜다. 이 관찰이 D-49의
  근거가 된다.

---

## D-48. ADK 0.8.0 설계 대조 — 자기 감사는 왜 실패했나

- **배경**: 설계 문서(`adk-prototype.md`+`adk-reference.md`, 3,288줄)대로 구현됐는지 점검
  목록을 만들고 감사 에이전트 여섯을 돌렸다. 결론은 "핵심 12 전부 통과, 남은 실패 1건".
  그 상태로 0.8.0을 릴리스했다. 직후 두 질문에 각각 실재하는 불일치가 걸렸고, 설계 원문과
  코드를 직접 대조하니 **19건**이 나왔다.
- **왜 못 잡았나 (셋)**:
  1. **항목이 없으면 영원히 통과다.** 설계가 두 번 못박은 "`~/.awl/engine/` 같은 사본을 두지
     않는다"는 점검 목록 전문에 **"사본"이라는 낱말조차 없었다.** 볼 항목이 없으니 통과였다.
  2. **골대를 옮겼다.** "서브에이전트를 하나도 안 띄워야 한다"를 통과로 찍으면서 근거에
     "awl-pipeline 계열은 의도적으로 안 건드림 — 다른 관심사"라고 적었다. 설계 원문은
     "`loop` 도 `pipeline` 도 단일 세션으로 끝난다"로 pipeline을 명시 포함한다. 실패한 부분을
     요구사항에서 빼낸 것이다.
  3. **선언한 검증법을 안 썼다.** 항목마다 `확인` 란에 "세션 수를 센다" 같은 **실행** 절차를
     적어놓고 실제 `근거`는 전부 코드 인용이었다. 설계가 요구한 "단계마다 실사용으로 검증"은
     여섯 단계 어디에도 기록이 없다.
- **결정**: 점검 목록(`docs/0.8.0/adk-checklist.md`)을 폐기하지 않고 그 자리에 정정을 붙였다.
  폐기하면 무엇이 왜 틀렸는지가 안 남는다. 문서 머리에 "근거로 쓰지 마라" 경고를 박고,
  골대를 옮긴 항목에 정정을 달았다.
- **앞으로**: 감사는 **구현하지 않은 주체가**, 코드 인용이 아니라 **실제로 돌려서** 한다.
- **대안(택하지 않음)**: 점검 목록을 다시 작성. 같은 주체가 다시 쓰면 같은 사각이 생긴다.

---

## D-49. 오케스트레이터 은퇴 — 레인당 한 세션 (0.8.4)

- **배경**: 설계(`adk-reference.md` §14 "세션당 레인 하나")가 오케스트레이터를 명시적으로
  없애기로 정했다. 시뮬레이션 실측이 근거였다 — 세 세션이 동시에 물어 정신없었고,
  오케스트레이터가 전체 토큰의 1.2%를 먹었다.
- **결정**: `awl-pipeline` 계열 스킬 8개, `awl pipeline-dispatch`/`hold-recheck`,
  `awl status --pipeline`, `.tasks` 일감 큐와 워처를 전부 제거했다. 레인은 남기되 세션은
  레인당 하나이고, 터미널을 여는 건 사람 몫이다.
- **핵심 근거 — 역할 이름은 세션을 나눌 근거가 아니다**: 사람 팀에서 역할을 나누는 이유는 한
  사람이 다 못 하기 때문인데 에이전트는 다 할 수 있다. 핸드오프가 잃는 게 컸다 — 넘기는 쪽은
  스펙을 만들며 얻은 이해·왜 이 티켓인지·이미 읽은 파일·사람이 게이트에서 한 말을 갖고 있는데
  받는 쪽은 일감 문서 한 장을 받는다. 나머지를 다시 만드느라 토큰을 쓰고, 다시 만든 이해가
  원래와 미묘하게 달라 실수도 난다.
- **남긴 것**: 세션을 더 띄우는 세 경우 — 편향 회피(`--review`), 동시성(레인), 오염 방지.
  그중에서도 띄우는 주체는 awl이 아니라 그 세션이다. awl은 판단도 스폰도 하지 않는다.
- **대안(택하지 않음)**: 오케스트레이터를 얇게 유지. 얇게 만들어도 핸드오프 손실은 그대로고,
  "awl이 스폰하지 않는다"는 원칙과 계속 부딪힌다.
- **비용**: 0.6~0.7에서 쌓은 무인 파이프라인 자산을 버렸다. 사용자가 이 대가를 알고 선택했다.

---

## D-50. 진입점 통일 — `awl run`(CLI)과 `/awl`(스킬) (0.8.2~0.8.3)

- **배경**: ADK 0.8.0의 스펙→티켓→4게이트 모델은 CLI로 만들어졌지만 **어디서도 안내되지
  않았다.** `awl init` 다음 단계도, `--help`도, README도 전부 `/awl-loop`(레거시 2게이트)만
  가리켰다. 실사용은 100% 옛 경로로 흘렀고 새 구조는 dark code였다.
- **결정**: 층을 나눠 각각 이름을 줬다.
  - `awl run "<목표>" [--strict|--auto] [--review] [--lanes ...]` — CLI가 요청·모드·레인을
    파일과 상태로 연다. **에이전트는 안 띄운다.**
  - `/awl` — 사람이 에이전트 안에서 부르는 스킬. `awl next`가 만든 지시를 따른다.
  - 리뷰어 스폰은 `--review`일 때 **그 세션**이 한다.
- **왜 모드를 CLI가 파싱하나**: 그 전에는 스킬이 목표 서술문에서 `--strict` 토큰을 문자열로
  긁어냈다. 목표에 그 낱말이 들어가면 오작동하는 구조였다.
- **`awl-loop`은 지우지 않고 명시 전용 레거시로 내렸다** — 이미 그 방식으로 돌던 작업이 있고,
  두 흐름은 게이트 번호의 뜻이 다르다(여기 게이트 2 = 워크아이템 완료 / ADK 게이트 2 = 티켓 착수).
- **Codex에도 `awl` 스킬을 새로 만들었다** — 승격이 Claude에만 적용되면 Codex 사용자는 ADK
  경로가 아예 없다.

---

## D-51. 엔진 사본을 두지 않는다 (0.8.7)

> D-44의 "설치된 엔진 vs 실행 바이너리" 쌍을 대체한다.

- **배경**: 설계가 두 번 못박았다 — "`~/.awl/engine/` 같은 사본을 두지 않는다. npm 패키지가
  유일한 엔진이다", "사본을 두면 어긋난다. 그와 같은 실수다". 그런데 `init`/`update`가 계속
  패키지의 `engine/`을 `~/.awl/engine`으로 복사하고 있었다. D-48이 말한 "항목이 없어서 통과"의
  대표 사례다.
- **결정**: 사본을 없앴다. `engineDir()`이 설치된 패키지의 `engine/`을 가리킨다. 딸려 있던
  것들도 같이 사라졌다 — `binary-vs-engine` 버전 쌍, doctor의 그 경고, `awl --version`의
  "Engine Template 불일치" 안내. 엔진을 올리는 방법은 `npm i -g` 하나뿐이다.
- **왜 사본이 나빴나**: 실행 바이너리는 0.9.x인데 홈의 엔진은 0.8.x인 상태가 가능했고, 그
  조합을 감지하는 코드·경고·갱신 명령이 전부 필요했다. 사본을 없애자 그 드리프트가 **구조적으로
  불가능**해졌다.
- **테스트 탈출구**: `AWL_ENGINE_DIR`로 엔진 위치를 지목할 수 있다. 사본을 만드는 게 아니라
  "어느 하나를 볼지"만 바꾸므로 "엔진은 전역 하나"와 충돌하지 않는다.
- **남긴 것**: `legacyEngineDir()` — 올라오는 설치에 남은 `~/.awl/engine` 잔재를 `awl remove`가
  계속 지운다.

---

## D-52. 무엇을 사람 층에, 무엇을 프로젝트 층에 둘 것인가 (0.8.8)

> D-11의 저장 위치 가정을 대체한다.

- **배경**: 감사가 "설계 §1의 세 층에는 `~/.awl/config.json`만 있는데 gotchas·rules·
  generations·templates가 전역에 있다"를 불일치로 잡았다. 코드를 옮길지 설계를 고칠지가
  갈렸다.
- **결정**: **코드를 두고 설계 문서를 고쳤다.** 이 넷은 사람 층에 있어야 "다음 프로젝트에서
  빈손으로 시작하지 않는다"가 성립한다. 저장소에 두면 저장소를 나가는 순간 사라진다.
  설계 §1 다이어그램에 넷을 명시하고 이유를 붙였다.
- **기록(`records/`)만 프로젝트 층인 이유도 같이 적었다**: 커밋·티켓과 1:1로 붙고 양이 크며,
  레인이 여럿일 때 주 워킹카피 하나를 심링크로 가리켜 실시간 공유한다. 사람 층에 두면
  레인마다 갈라져 정리 때 병합해야 하는데, 그게 레인 설계가 피하려는 구조다.
- **같은 판단을 적용한 둘**: 운영 명령(`init`·`doctor`·`remove`·`status`)은 설계가 이름을
  안 정했을 뿐 없으면 개념 명령을 쓸 수 없다 — "설계에 이름이 없다"와 "있으면 안 된다"를
  구분해 §17에 표로 정리했다. 패키지명은 초안의 `@midasit-dev/awl` 대신 실제 배포명
  `agent-work-loop`으로 설계를 맞췄다(지금 바꾸면 npm에서 별개 패키지가 되어 기존 사용자가
  갱신을 못 받는다).

---

## D-53. 워크아이템을 티켓으로 대체하되 전면 리네임은 하지 않는다 (0.8.5)

- **배경**: 설계는 `workitem`을 `ticket`으로 대체했다(`adk-reference.md`에 workitem 0회).
  그런데 `src`+`tests`에 workitem 참조가 775개다.
- **결정**: 전면 리네임을 **하지 않았다.** 대신 설계가 실제로 요구하는 것만 좁게 했다.
  - `awl work`를 도움말에서 내렸다(hidden). 정식 경로는 `awl doc new spec` → `awl tickets
    derive` → `awl commit <ticket-id>`라 work 없이 돈다.
  - **게이트 2의 이중 의미를 막았다.** 레거시(layer 없음) = 워크아이템 **완료**, ADK
    (`layer:'ticket'`) = 티켓 **착수**로 정반대인데 optional 필드 하나로만 갈렸다.
    `ticket`을 주면서 `layer`를 빠뜨리면 거부한다 — 추측해서 고쳐주지 않는다.
  - `awl record`가 `workitem` 대신 `ticket`만으로도 기록을 받는다(0.9.3). ADK 경로에는
    워크아이템이 없어서 **게이트를 기록할 수 없던 배선 결함**이었다.
- **왜 전면 리네임을 안 했나**: `commit`·`evolve`·`metrics`는 사용자 작업을 잃지 않게 지키는
  가장 고위험 경로다. 775개 참조를 훑는 리팩터가 거기서 버그를 만들면 얻는 것보다 잃는 게
  크다. 어휘 통일은 가치가 있지만 그 값을 치를 자리가 아니다.

---

## D-54. 규칙 개수 상한을 두지 않는다 (0.8.6)

- **배경**: `RULE_LOAD_LIMIT = 15`가 규칙이 15개를 넘으면 경고했다. 설계 §12는 정반대를
  말한다 — "초반에는 쌓기만 한다. **상한도 승격도 두지 않는다.** 지금 제약이 몇 개나 쌓일지,
  어떤 종류가 많을지, 검사 가능한 비율이 얼마일지 모른다."
- **결정**: 개수 경고를 없앴다. 관찰되지 않은 숫자로 조이면 엉뚱한 데를 조인다. 정리는
  개수가 아니라 **신호**로 한다 — `awl backlog`가 "3회 반복된 승격 후보 30건 초과"를 낸다.
  누적이 아니라 증분이라 정리하면 다시 0부터 센다.
- **`checkRuleLoadLimit` 시그니처는 남겼다** — 호출부(`runRules`)의 "여기서 정리 신호를 본다"는
  자리가 사라지면 나중에 backlog 연결을 어디에 붙일지 알기 어려워진다.
- **승격(`awl rules promote`)은 남겼다**: 설계의 "승격"은 clarification→decision을 가리키고
  이건 gotcha→규칙이라 다른 축이다.

---

# Windows 리스크 목록 (macOS에서만 검증함 — Windows 검증 시 체크리스트로 사용)

이 프로젝트는 여전히 macOS에서만 실사용으로 검증한다. 아래는 Windows에서 깨질 수 있는
지점과 대비다. Windows에서 사람이 검증할 때 하나씩 확인한다.

> **리눅스에서 실제로 하나 터졌다(0.9.1).** CI(ubuntu)에서 포트 lease가
> `listen EINVAL: fe80::…`로 전부 죽었다. 원인은 링크로컬 IPv6에 zone index(`%eth0`) 없이
> 바인드한 것이고, macOS는 받아줘서 로컬로는 안 보였다. **"POSIX면 다 같다"가 아니다** —
> 이 목록의 대비들도 macOS에서 통과했다는 것 이상은 증명하지 못한다. 링크로컬은 점유 판정
> 대상에서 빼고 `EINVAL`을 "그 주소에선 아무도 안 듣는다"로 받아 넘기게 고쳤다.

> **Node 버전 차이도 하나 터졌다(0.9.1).** 테스트 픽스처가 `--experimental-strip-types`로
> `.ts`를 직접 돌리는데 그 플래그는 Node 22.6+ 전용이라, `engines`가 `>=18`인데도 Node 18
> 잡이 종료코드 9로 죽었다. 그 테스트만 런타임 조건부로 건너뛴다.
