...
describe('buildAutoFeedbackWhat / commandNameFromArgv — 순수 함수', () => {
  ...
  it('argv 에서 옵션이 아닌 첫 인자를 명령 이름으로 본다', () => {
    expect(commandNameFromArgv(['node', 'cli.js', 'record', 'gate', '--json', '{}'])).toBe(
      'record',
    );
  });
...
describe('recordAutoFeedback — 통합', () => {
  ...
  it('프로젝트를 찾을 수 있고 기본값(켜짐)이면 awl-feedback 레코드를 남긴다', async () => {
    await recordAutoFeedback(new Error(`${root}/src/foo.ts 에서 실패`), [
      'node',
      'cli.js',
      'doctor',
    ]);
  ...
  it('프로젝트 루트를 못 찾으면(프로젝트 밖) 조용히 아무것도 안 한다', async () => {
    ...
    await expect(
      recordAutoFeedback(new Error('실패'), ['node', 'cli.js', 'doctor']),
  ...
  it('Error 가 아닌 값(문자열 throw 등)도 크래시 없이 처리한다', async () => {
    await expect(
      recordAutoFeedback('plain string error', ['node', 'cli.js', 'doctor']),
    ).resolves.toBeUndefined();
    const records = readRecords(root, { type: 'awl-feedback' });
    expect(records).toHaveLength(1);
