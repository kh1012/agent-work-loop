...
describe('collectPipelineLaneGroups — 교차 레인 롤업(pipeline-status-view AC-01)', () => {
  it('레인 2개를 각각 순회해 레인별 workitem 상태를 롤업한다', () => {
    ...
    seedLane(root, 'fe', { plan: ['loginㅍ.md'], exec: [], review: [] });
    seedLane(root, 'be', { plan: [], exec: [], review: ['migrate.pass.md'] });
...
describe('runStatus --pipeline 교차 레인(pipeline-status-view AC-02/03)', () => {
  ...
  it('AC-02: 레인 없으면 기존 단일 .tasks/ 평탄 lanes[] 로 폴백한다', () => {
    ...
    const j = JSON.parse(capture(() => void runStatus({ json: true, pipeline: true })));
    // 폴백: 평탄 {name,status} (기존 스키마 불변). workitems 가 생기면 폴백이 안 걸린 것.
    expect(j.lanes).toEqual([{ name: 'freshwi', status: 'pending' }]);
    expect(j.lanes[0].workitems).toBeUndefined();
  });
});
