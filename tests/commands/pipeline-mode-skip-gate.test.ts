...
describe('pipeline-gate-density — 3단계 설명·오해방지·매핑 리네임 (AC-02)', () => {
  it('gate-medium: 권장값 자동 진행 + critical(high) 최종 요약·기록으로 설명한다', () => {
    const text = skill();
  ...
  it('오해방지: awl 게이트=판단 정지점, 도구 권한 아님(--dangerously-skip-permissions 와 다른 층위)', () => {
    const text = skill();
  ...
  it('gate-high: 매 게이트 사람 승인(개입 최대), gate-low: critical 자율(개입 최소)', () => {
    const text = skill();
    // gate-high — 매 게이트 승인·개입 최대
    expect(text).toMatch(/매 게이트 사람에게 물어|매 결정.*사람 승인/);
    expect(text).toContain('개입 최대');
    // gate-low — critical 포함 최종 문의 없이 자율·개입 최소
    expect(text).toContain('최종 문의 없이 자율');
    expect(text).toContain('개입 최소');
  });

  it('mode 매핑이 graded 볼드 헤더로 리네임됨 — 옛 볼드 헤더(gate/skip-gate/auto)는 사라졌다', () => {
    const text = skill();
    // 새 graded 볼드 헤더 present
    expect(text).toMatch(/\*\*`gate-high`\*\*/);
    expect(text).toMatch(/\*\*`gate-medium`\*\*/);
    expect(text).toMatch(/\*\*`gate-low`\*\*/);
    // 옛 볼드 모드 헤더의 뮤테이션 저항 — "(=기존 `skip-gate`)" 전이 주석은 볼드가 아니라 통과
    expect(text).not.toMatch(/\*\*`gate`\*\*/);
    expect(text).not.toMatch(/\*\*`skip-gate`\*\*/);
