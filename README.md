...
**awl 자체는 판단하지 않습니다.** LLM을 호출하지 않고, 파일과 상태만 결정적으로 다룹니다. 무엇을 배우고 어떤 규칙을 세울지는 이미 설치된 에이전트(Claude Code, Codex)가 스킬로 판단합니다. 에이전트가 머리라면 awl은 손발입니다. 그래서 awl의 모든 동작은 예측할 수 있고 테스트할 수 있습니다.
...
스킬이 이 **작업 루프**를 따릅니다 — 워크아이템 하나의 생애입니다(조사부터 게이트 2까지). **awl은 손발일 뿐, 판단은 에이전트가 합니다.** (여러 워크아이템을 격리된 레인에서 동시에 굴리는 층은 아래 [오케스트레이션 파이프라인](#오케스트레이션-파이프라인--여러-레인을-나란히)에서 다룹니다.)
...
앞의 작업 루프가 워크아이템 하나의 생애라면, 오케스트레이션 파이프라인은 여러 워크아이템을 격리된 채로 동시에 굴리는 층입니다. `awl work switch`가 한 워킹트리 안에서 작업을 순차로 오가는 것이라면, 파이프라인은 작업마다 전용 워크트리(레인)를 두고 나란히 진행합니다.
...
**로드맵**: 목표 하나를 던지면 오케스트레이터가 레인을 쪼개고 세션을 자동으로 띄우는 auto-spawn은 아직 없습니다(설계 보류). 지금은 위처럼 사람이 세션을 엽니다.
...
npm에 `awl` 패키지가 이미 있어, 패키지 이름은 `agent-work-loop`로 배포하고 명령어 이름만 `awl`로 씁니다. 둘이 다른 것은 의도된 선택입니다.
