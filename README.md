...
> **팀 내부 설치(지금 당장 필요하면)**: 이 README를 작성하는 시점 기준으로 npm 레지스트리의 `agent-work-loop`는 아직 `0.0.0`(이름 예약용 placeholder)만 배포돼 있어 `npm install -g agent-work-loop`로는 동작하는 CLI가 설치되지 않습니다. 지금 바로 써야 한다면 이 저장소를 클론해 직접 빌드·링크하세요.
> ```bash
> git clone <이 저장소> && cd agent-work-loop
...
질문 없이 감지된 값으로 바로 진행하려면 `awl init --yes`를 씁니다. 실제 출력(스크래치 프로젝트, `--yes`):
