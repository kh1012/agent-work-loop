...
|---|---|---|
| 담는 것 | 엔진(스킬 템플릿), 규칙, 교훈(delta), 기록(records), 등록된 프로젝트 목록 | 이 프로젝트의 설정(config.json), 지금 루프 위치(state.json) |
...
2. 워크아이템이 끝나면(게이트 2 통과) `awl evolve`가 이 기록들을 모아 재사용 가능한 교훈(delta)으로 추출합니다. `awl deltas`로 볼 수 있습니다.
