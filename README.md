...
무관한 작업 여러 개를 병행하고 싶으면 Claude Code에서 `/awl-pipeline <레인명> <mode>`, Codex에서 `$awl-pipeline <레인명> <mode> [--poll <interval>]`을 씁니다.
...
**Codex idle polling**: 미래의 plan이 나중에 들어오는 레인은 `--poll <interval>`을 선택적으로
붙입니다. 예를 들어 `--poll 30m`은 현재 chat의 native Scheduled task로 30분마다 lane
marker를 확인합니다. Scheduled capability가 없는 Codex surface에서는 활성화됐다고
가정하지 않고, goal·sleep·shell watcher·cron으로 흉내 내지 않습니다.
