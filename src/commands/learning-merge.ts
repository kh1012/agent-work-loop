 ...
 *
 * 왜 필요한가:
 *   --isolated 는 AWL_HOME 을 로컬 .awl/home 으로 돌린다. gotchas/rules/generations
 *   (학습)이 globalRoot() 단일 스위치를 따라 로컬로 격리된다. .awl/home 은 gitignored
 *   라 teardown 때 워크트리째 삭제 → 격리 레인이 쌓은 학습이 전역으로 안 이어지고
 *   소멸했다("격리하되 학습은 이음" 위반).
 *
 *   records 는 이 격리 대상이 아니다(WI-G17a) — project-local(.awl/records/)이라
 *   AWL_HOME 스위치와 무관하고, 레인 워크트리는 심링크로 실시간 공유한다(lane.ts
 *   linkLaneRecords, WI-G17b) — 병합이 아니라 처음부터 같은 파일이라 teardown 때
 *   할 일이 없다.
 *
 * 기법 (a) teardown 병합 — (b) always-global-write 는 전역 동시쓰기 락을 전제하는데
 *   그 락 신설은 concurrency(P0) 몫이라, 여기서는 teardown 때 로컬→전역으로 병합만 한다.
 *   state 는 병합하지 않는다 — 로컬 격리·폐기 그대로다.
 *
 ...
 * 대상 — records/state 는 이 함수 스코프 밖이다. records 는 project-local(.awl/records/,
 * WI-G17a)이라 애초에 격리 home 아래 안 쌓인다 — 레인은 심링크로 실시간 공유하므로
 * (lane.ts linkLaneRecords, WI-G17b) 병합이 아니라 "이미 같은 파일"이다. 부작용은
 * toRoot 아래 쓰기뿐.
 */
...
/**
 * teardown 진입점 — 격리 home 의 학습(gotchas/rules/generations)을 전역으로 넘긴다.
 * 워크트리 삭제 전에 호출한다. records 는 여기서 안 다룬다(WI-G17b — project-local +
 * 심링크 공유로 이미 실시간 반영되어 있다, 병합할 게 없다). 병합할 게 없거나(격리 home
 * 부재) 출발=목적(같은 store)이면 null(할 일 없음).
 */
