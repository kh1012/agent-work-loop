...
export async function runWorkNew(
  ...
  if (opts.skipBaseline) {
    ...
    if (loaded.config) {
      const report = await runVerifyChecks(loaded.config.verifications, verifyRoot, {
        bail: false,
      });
      try {
...
export async function runWorkDone(id: string, opts: { force?: boolean } = {}): Promise<void> {
  ...
  // 격리(.awl/home) 학습을 전역으로 병합한다 — 워크트리/홈 삭제·완료 전에.
  // --isolated 는 worktree 유무로 홈 위치가 갈린다(work new): 워크트리 wi 는
  // worktree/.awl/home, 비워크트리 격리 wi 는 root/.awl/home. 격리가 아니면(.awl/home
  // 부재) no-op. 멱등이라 --force 재시도에도 중복되지 않는다. records 는 여기서 안
  // 다룬다(WI-G17b — project-local + 접미사로 이미 충돌 없이 공존한다, 병합할 게
  // 없다). 병합이 실패하면(전역 쓰기 오류 등) 깔끔히 중단해 학습을 보존한다 — 삭제
  // 전이라 재시도로 복구된다.
